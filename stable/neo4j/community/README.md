...
```bash
$ helm install stable/neo4j/community
```
...
```bash
# run the following command if you get an error regarding not being able to find tiller
# helm init
$ helm install my-neo4j stable/neo4j/community --set neo4jPassword=mySecretPassword
```
...
```bash
$ helm install my-neo4j --set core.numberOfServers=5,readReplica.numberOfServers=3 stable/neo4j/community
```
...
```bash
$ helm install --name neo4j-helm -f values.yaml stable/neo4j/community
```
